...
def dbscan(df):
    st.header("Training the Model in DBScan")
    df['Start Date'] = pd.to_datetime(df['Start Date'], format='%m/%d/%Y')
    ...
    crime_category = st.selectbox("Choose a column", [
    ...
    X = scaler.fit_transform(df_crime)
...
def main():
    ...
    menu = ['Data Exploration', 'Feature Description', 'Crime Trends',
            'Show Clusters', "Show Map", "Preprocessing", "Training", "Prediction", "DBScan"]
